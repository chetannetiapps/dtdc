...
<div class="track-another-shp"><?php $block = module_invoke('global_tracking', 'block_view', 'global_tracking_block');
        print render($block['content']);?></div></div>
</div><?php }?>
...
//print_r($trackRecord['Event']);
   // foreach ($trackRecord['Event'] as $key => $value) {
   //        $eventname[] = $value['EventName'];
   //        }
   //       // print_r($eventname);
   //        if (in_array("Proof of Delivery", $eventname)) {
   //          print "Proof of Delivery";
   //        }

   //         else {
   //           print $trackRecord['Event'][$noofevents - 1]['EventName'];
   //         }
    ?>
    <?php 
    ...
    ?>
			...
			<td nowrap="nowrap"><strong>Status</strong><br> <?php print $track_id == 1 ? 'Delivered' : $trackRecord['Event'][$noofevents - 1]['EventName']; ?> </td>
            <td nowrap="nowrap"><strong>Date Time<br></strong><?php print $trackRecord['Event'][$noofevents - 1]['FormatedEventDateTime']; ?></td>
        </tr>
        <tr class="altRow">
	<!--		<td valign="top"><strong>Origin</strong><br> <?php // echo $Consignorcountryname .', '.$trackRecord['Event'][0]['OriginEntity'].', '.$trackRecord['Event'][0]['UpdateEntityCountryName']; ?></td> -->
      ...
      <td valign="top"><strong>No pcs</strong><br> <?php !(empty($trackRecord['Event'][0]['Pcs'])) ? print $trackRecord['Event'][0]['Pcs'] : print 'NA'; ?> </td>
        ...
        <tr class="altRow">
		<!--	<td valign="top"><strong>Destination</strong><br> <?php // !(empty($Consigneecountryname)) ? print $Consigneecountryname : !(empty($trackRecord['Event'][0]['Destination'])) ? print $trackRecord['Event'][0]['Destination'] : NA; ?></td> -->
  ...
  <div class="panel panel-default separate" id="panel5">
    ...
    <div class="panel-collapse collapse" id="history<?php print $i; ?>">
      ...
      <?php   
        if($track_id == 1){
        ?>
         <img src="/sites/default/files/pictures/img-rev<?php print $track_id; ?>.png">
       <?php } else { ?> 
      ...
      <?php 
        ...
        <div class="col-md-12 panel-even">
          ...
          print $history['EventID'] == 1 ? 'Delivered' : $history['EventName']; 
          ?><span class="book_icon"></span></div>
          <div class="col-md-4">
			  <?php
